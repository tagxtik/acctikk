...
class Jdetailmod extends ChangeNotifier {
    // Private list of items
  List<JornalModelDetails> _items = [];

  // Public getter to access the list of items
  List<JornalModelDetails> get items => _items;

  // Method to set items directly (like a setter)
  void setItems(List<JornalModelDetails> newItems) {
    _items = newItems;
    notifyListeners(); // Call notifyListeners if using ChangeNotifier
  }
  List<int?> _selectedRadioIndexes = []; // Stores selected radio button for each item
  ...
  Future<void> loadItems() async {
    ...
    String? itemsJson = prefs.getString('jdetailsitems');

    if (itemsJson != null) {
      try {
        List<dynamic> itemsList = jsonDecode(itemsJson);
        _items = itemsList.map((itemJson) {
          return JornalModelDetails.fromJson(jsonDecode(itemJson));
        }).toList();
      } catch (e) {
  ...
  Future<void> addJornalItem(JornalModelDetails newItem) async {
    ...
    _items.add(newItem);
    List<String> updatedItemsJson = _items.map((item) => jsonEncode(item.toJson())).toList();
    await prefs.setString('jdetailsitems', jsonEncode(updatedItemsJson));
  ...
  double calculateSumDebit() {
    ...
    return double.parse(totalDebit.toStringAsFixed(3)); // Round to 2 decimal places
  }
  ...
  double calculateSumCredit() {
    ...
    return double.parse(totalCredit.toStringAsFixed(3)); // Round to 2 decimal places
  }
  ...
  double calculateDifference() {
    ...
    return double.parse(difference.toStringAsFixed(3)); // Round to 2 decimal places
  }

  // Inside the Jdetailmod provider
Future<void> deleteItem(int itemIndex, String itemType) async {
  if (itemType == "Credit") {
    final creditItems = _items.where((item) => item.type == "Credit").toList();
